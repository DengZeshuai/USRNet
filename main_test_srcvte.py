...
def main():
    ...
    assert len(L_paths) == 1200, "wrong number of test images"
    ...
    for idx, img in enumerate(L_paths):
        ...
        # (1) img_L
        # ------------------------------------
        # assert os.path.exists(img), "images do not exists"
        img_name, ext = os.path.splitext(os.path.basename(img))
        if testset_name == 'srcvte':
            video_name = os.path.basename(os.path.dirname(img))
            logger.info('{:->4d}--> video:{:>4s} frame: {:>10s}'.format(idx+1, video_name, img_name+ext))
        else:
        ...
        if save_L:
            if testset_name == 'srcvte':
                ...
                util.imsave(util.single2uint(img_L), os.path.join(save_path, img_name+'_LR_x'+str(sf)+'.png'))
            else:
